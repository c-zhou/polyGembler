...
public class TwoPointAnalysis extends RFUtils {
	...
	private class RfCalculator implements Runnable {
			...
			try {
				...
				for(int i = 0; i<best_n; i++) {
					...
					for(int j=0; j<best_n; j++) {
						dj = data_j[j];
						if(dj==null) continue;
						stat = conj ? sum(comns(di.p0, di.p1, dj.p0, dj.p1, false), comns(di.m0, di.m1, dj.m0, dj.m1, false)) : 
							max( sum(comns(di.p0, di.p1, dj.p0, dj.p1, true), comns(di.m0, di.m1, dj.m0, dj.m1, true)), 
									sum(comns(di.p0, di.p1, dj.m0, dj.m1, true), comns(di.m0, di.m1, dj.p0, dj.p1, true)) );
		...
		private int[] comns(Map<Integer, Set<Integer>> a0, Map<Integer, Set<Integer>> a1,
				Map<Integer, Set<Integer>> a2, Map<Integer, Set<Integer>> a3, boolean permutate) {
			// TODO Auto-generated method stub
			int[][][] comns = new int[4][][];
			comns[0] = comns(a0, a2);
			comns[1] = comns(a0, a3);
			...
			System.arraycopy(stat, 0, sels, 0, 4);
			maxc = max(stat);
			sumc = sum(stat);
			
			if(!permutate) return sels;
