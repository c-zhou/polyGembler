									...
									outerloop:
										while(z<asz) {
											...
											dobj = target_as.getScore()+
													(target_qstart>source_qend?TraceableAlignmentSegment.gap_open:0)-
													(target_qstart>source_qend?0:(source_qend-target_qstart)*TraceableAlignmentSegment.match_score);
											...
											for(int v=z+1; v<asz; v++) {
												...
												if(tmp_qstart>source_qend && 
													...
													break;
												tmp = tmp_as.getScore()+
														(tmp_qstart>source_qend?TraceableAlignmentSegment.gap_open:0)-
														(tmp_qstart>source_qend?0:(source_qend-tmp_qstart)*TraceableAlignmentSegment.match_score);
												if(tmp>dobj) {
													// update selected
													dobj = tmp;
													target_as = tmp_as;
